...
export function RejectDialog({ id, disabled = false }: RejectDialogProps) {
  ...
  async function rejectHandler() {
    try {
      ...
      toast({
        title: 'Reject Copyright Document Success',
      });

      setOpen(false);
    } catch (error) {
      toast({
        title: 'Reject Copyright Document Failed',
        variant: 'destructive',
      });
    } finally {
  ...
  return (
    ...
    >
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Reject copyright document</DialogTitle>
        </DialogHeader>
        <div>
          <p>Do you want to reject this copyright document?</p>
        </div>
        <DialogFooter>
          <Button
            variant="destructive"
